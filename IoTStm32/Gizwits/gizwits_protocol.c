...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagGPIO1)
    {
        info->event[info->num] = EVENT_GPIO1;
        info->num++;
        dataPoints->valueGPIO1 = gizStandardDecompressionValue(GPIO1_BYTEOFFSET,GPIO1_BITOFFSET,GPIO1_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagGPIO2)
    {
        info->event[info->num] = EVENT_GPIO2;
        info->num++;
        dataPoints->valueGPIO2 = gizStandardDecompressionValue(GPIO2_BYTEOFFSET,GPIO2_BITOFFSET,GPIO2_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueGPIO1 != cur->valueGPIO1)
    {
        GIZWITS_LOG("valueGPIO1 Changed\n");
        ret = 1;
    }
    if(last->valueGPIO2 != cur->valueGPIO2)
    {
        GIZWITS_LOG("valueGPIO2 Changed\n");
        ret = 1;
    }
    ...
    if(last->valueADC1 != cur->valueADC1)
    {
        if(currentTime - lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueADC1 Changed\n");
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(GPIO1_BYTEOFFSET,GPIO1_BITOFFSET,GPIO1_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGPIO1);
    gizStandardCompressValue(GPIO2_BYTEOFFSET,GPIO2_BITOFFSET,GPIO2_LEN,(uint8_t *)devStatusPtr,dataPoints->valueGPIO2);
    ...
    devStatusPtr->valueADC1 = gizY2X(ADC1_RATIO,  ADC1_ADDITION, dataPoints->valueADC1); 
